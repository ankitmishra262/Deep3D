...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.act1(x)
        pred1 = self.deconv1(x)
        pred2 = self.act2(pred1)
        pred2 = self.deconv2(pred2)
        pred3 = self.act3(pred2)
        pred3 = self.deconv3(pred3)
        pred4 = self.act4(pred3)
        pred4 = self.deconv4(pred4)
        pred5 = self.act5(pred4)
        pred5 = self.deconv5(pred5)

        feats = pred1 + pred2 + pred3 + pred4 + pred5
        feats = self.feat_act(feats)

        up = self.deconv6(feats)
        up = self.relu6(up)
        up = self.conv6(up)
